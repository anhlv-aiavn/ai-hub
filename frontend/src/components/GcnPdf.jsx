...
import { pageImageUrl, getPageInfo } from "../api.js";

// Đối soát: ảnh trang PDF render ở server (1 <img>/trang, lazy-load). Trang 1-indexed;
// backend page index 0-indexed → n = p-1.
export default function GcnPdf({ gcnId, page = 1, onPageChange }) {
    ...
    <div className="pdf-pane">
      <div className="pdf-rail">
        {Array.from({ length: numPages }, (_, i) => i + 1).map((p) => (
          <div key={p} className={`thumb ${p === cur ? "active" : ""}`}
            ...
            <img loading="lazy" src={pageImageUrl(gcnId, p - 1, 200)} alt={`Trang ${p}`} />
      ...
      <div className="pdf-main">
        ...
        <div className="pdf-canvas-wrap">
          <img className="pdf-page-img zoomable" key={cur} src={pageImageUrl(gcnId, cur - 1, 1400)}
            alt={`Trang ${cur}`} title="Bấm để mở ảnh trang ở tab mới"
            onClick={() => window.open(pageImageUrl(gcnId, cur - 1, 2200), "_blank", "noopener")} />
        </div>
      </div>
    </div>
  );
